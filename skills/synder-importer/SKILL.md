...
2. **Connect your accounting software** — link QuickBooks Online or Xero via OAuth in the web UI
3. **Generate an API key** — go to **Account → API Keys**, click **Generate**. Copy the token immediately — it's shown only once.

Store the token in environment variable `IMPORTER_API_TOKEN`:
...
```
Base URL: https://importer.synder.com/api/v1
Auth: Authorization: Bearer $IMPORTER_API_TOKEN
```
...
- `dryRun=true` (default): Returns proposed mapping without importing. Check `missingRequired` array — if empty, safe to import.
- `dryRun=false`: Maps AND imports in one call.

Response fields: `proposedFields` (array of matches), `missingRequired` (unmatched required fields), `totalFieldsMapped`.
